...
(well aware that the bottom gif does look weird with the path starting from the corner instead of straight along - but the distance is still the same since cost to move between grids is only 1 unit. It annoys me too. Might make diagonal movement cost .5 more than adjacent movements another time (maybe))
